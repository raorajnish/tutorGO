...
| 12.4 | Not built | |
| 12.5 | **Built** | `StudyResource` model, course-scoped rather than batch-scoped (confirmed with the user — the same notes serve every batch of a course, so per-batch would just mean duplicates drifting apart), with an optional `subjectId`. `POST/GET /study-resources` + `POST /study-resources/upload` (OWNER/ADMIN/FACULTY), `GET /portal/study-resources` (student, scoped to their own course, never a request param). Deleting a resource removes the Cloudinary asset too, not just the row. Frontend: new "Study material" nav item for staff, and a "Study material" item in the portal's My learning nav — the latter driven by a live `hasStudyResources` flag on `/auth/me` so it's hidden until the student's course actually has something in it (confirmed with the user). **Not built, deliberately**: the `RESOURCE_UPLOADED` notification from the original plan — it depends on a new `MessageTemplateType` enum value plus per-institute WhatsApp template seeding, a bigger blast radius than the library itself, and the user explicitly said no notification was needed. Verified live: subject-not-taught-on-course rejected, cross-institute course access rejected, delete removes the Cloudinary asset, and the nav flag flips true/false as resources are added/removed. |
| 12.6 | **Built** | TOTP via `otplib` (pinned to v12 — v13 is a rewrite with a plugin-based crypto/base32 API that doesn't match the classic `authenticator` singleton the plan assumed; v12 has it and is stable) + QR via `qrcode`. `User.mfaSecret` (encrypted with the existing `lib/crypto.ts`), `mfaEnabledAt`, `mfaBackupCodes` (hashed, one-time-use). `POST /auth/mfa/setup|confirm|disable`, and `POST /auth/login` now returns `{mfaRequired, challengeToken}` instead of a token when enabled; `POST /auth/mfa/verify` exchanges a code (TOTP or backup) for the real session. SuperAdmin escape hatch: `POST /platform/users/:id/mfa/disable`, reason required, audit-logged — surfaced on a new platform-wide `/platform/users` directory (Users nav entry) rather than duplicated across every institute team list, since institute pages don't even show FACULTY/RECEPTION. Frontend: Profile → Security gets the real setup/disable flow (replacing the old "Coming soon" placeholder), login page gets a second step (code or backup code) that only appears for accounts with it enabled. **Two decisions expanded beyond the original plan, confirmed with the user before building**: (1) eligible roles widened from OWNER/ADMIN to every staff role (OWNER/ADMIN/ACCOUNTANT/FACULTY/RECEPTION) — STUDENT/SUPERADMIN stay excluded, the latter deliberately (it's the account that has to stay reachable to disable everyone else's MFA). (2) Opt-in confirmed (not mandatory), toggled from Profile. Verified live end-to-end including the exact boundaries that matter: wrong TOTP code rejected, correct code accepted, a used backup code cannot be replayed, disable requires the correct current password, the SuperAdmin hatch requires a reason and is audit-logged, and an ineligible role (SUPERADMIN) is correctly forbidden from even starting setup. One real lesson from verification, not a bug: manually testing TOTP by hand across several separate `curl` calls is genuinely flaky near a 30-second window boundary — generate-and-submit has to happen as close to one atomic step as possible. |
...
| 12.11 | **Built** | This row was stale — the feature landed without this table being updated at the time; corrected after a direct code check. `MaintenanceWindow` model (GLOBAL or INSTITUTE scope) + `lib/maintenance.ts`'s `deriveStatus()` (upcoming/active/ended/cancelled computed live from `startAt`/`endAt`/`cancelledAt`, never stored — the same "derive, don't store" rule every other status field in this codebase follows), with a 15s in-process cache invalidated immediately on any write. `authenticate` calls `getBlockingWindow()` and rejects every non-SUPERADMIN request with `503 MAINTENANCE_ACTIVE` while a window is genuinely active — the same choke point session revocation (§12.2) and suspension (§12.10) use. Full CRUD in `platform.ts`: schedule (rejects overlapping windows per scope, validates `endAt > startAt` and both in the future), list (derived status per row), cancel — all audit-logged. Frontend: a SuperAdmin scheduling/management page, a blocking full-page state for whoever hits it, and a pre-start countdown banner (`getNoticeWindow()` — prefers an already-active window over the soonest upcoming one). |
