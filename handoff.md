...
### Dev-database schema changes now use tracked migrations, not `prisma db push`
**This note is reversed as of 2026-09-06** — `prisma/migrations/` now exists, starting with `20260906022419_init` (a baseline generated from the schema as it stood that day, applied via `prisma migrate resolve --applied` since the dev DB already matched it via `db push`). `db push` was the right call for solo pre-launch iteration; it stopped being right the moment real migration history was worth having for review/rollback ahead of a first production deploy.

**From here on**: `npm run prisma:migrate` (`prisma migrate dev --name <description>`) for every schema change in dev — it now works correctly and creates a new file under `prisma/migrations/`. `npm run prisma:deploy` (`prisma migrate deploy`) is what a fresh clone or production applies. Do not reach for `prisma db push` again outside of a genuinely throwaway local experiment you intend to discard — it has no history and can't be reviewed or rolled back, which is exactly what tracked migrations exist to fix.

Note: `prisma migrate dev`'s shadow-database step fails locally with `permission denied to create database` (the local Postgres role lacks `CREATEDB`) — if you hit that, either grant the role `CREATEDB`, or fall back to `prisma migrate diff --from-empty --to-schema prisma/schema.prisma --script` + `prisma migrate resolve --applied` the way the initial migration was created.
...
- **Pagination**: almost every list in the app loads in full today — only the Audit Log is actually paginated. When a new list needs it (student directory and payment/attendance history are the likely first candidates as institutes grow), use `lib/pagination.ts`'s `DEFAULT_PAGE_SIZE` (20) on the backend and the shared `components/ui/Pagination.tsx` on the frontend — don't roll a one-off Previous/Next pair like the Audit Log originally did before this component existed. 20 is the default for a normal table; a dense, filter-first tool can override it the way the Audit Log's `AUDIT_LOG_PAGE_SIZE` (50) does.
